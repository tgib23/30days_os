...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	if (strcmp(cmdline, "mem") == 0) {
		...
		if (cmd_app(cons, fat, cmdline) == 0) {
			...
			putfouts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "Bad command.", 12);
			cons_newline(cons);
			cons_newline(cons);
		}
	}
...
void cmd_mem(struct CONSOLE *cons, int memtotal)
{
	...
	putfouts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, s, 30);
	cons_newline(cons);
...
void cmd_type(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		/* ファイルが見つからなかった場合 */
		putfouts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "File not found.", 15);
		cons_newline(cons);
	}
	cons_newline(cons);
	return;
